...
bool validateMetricId(const string& metricId) {
    ...
    static const std::regex str_expr(
            "^(counter|histogram)[.]_*[a-z]+[a-z_0-9]*[.]value_*[a-z]+[a-z_0-9]*");

    if (!std::regex_match(metricId, str_expr)) {
        LOGE("Metric Id does not follow convention: %s\n", metricId.c_str());
        return false;
    }
