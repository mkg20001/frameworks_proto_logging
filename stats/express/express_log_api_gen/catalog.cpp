...
vector<fs::path> buildConfigFilesList(const fs::path& configDirPath) {
...
bool readMetrics(const fs::path& cfgFile, unordered_map<string, ExpressMetric>& metrics) {
...
bool generateMetricsIds(const unordered_map<string, ExpressMetric>& metrics,
    ...
    for (const auto& metric : metrics) {
        ...
        if (metricsIds.find(metric.first) != metricsIds.end()) {
            LOGE("Detected dublicate metric id %s.\n", metric.first.c_str());
            return false;
        } else {
            int64_t hashId = hash64(metric.first);
            ...
            pbMessage.set_id(metric.first);
            pbMessage.set_hash(hashId);
            pbMessage.set_type(metric.second.type());

            metricsIds[metric.first] = pbMessage;
        }
    }
