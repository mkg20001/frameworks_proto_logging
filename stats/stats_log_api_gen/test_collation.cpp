...
protected:
    void SetUp() override {
        if (GetParam()) {
            ...
            mExtensionAtoms = ExtensionAtoms::descriptor();
        } else {
            ...
            mExtensionAtoms = mFileDescriptor->FindMessageTypeByName("ExtensionAtoms");
        }
    }
    ...
    const Descriptor* mExtensionAtoms;
};
...
TEST_P(CollationTest, CollateExtensionAtoms) {
    Atoms atoms;
    const int errorCount = collate_atoms(mExtensionAtoms, "test_feature", &atoms);
