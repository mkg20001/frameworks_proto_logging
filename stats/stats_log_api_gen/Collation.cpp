...
const int PULLED_ATOMS_START = 10000;
const int PULLED_ATOMS_END = 100000;
...
AtomDecl::AtomDecl(int c, const string& n, const string& m, const AtomType& a)
...
static AtomType getAtomType(const FieldDescriptor* atomField) {
    AtomType atomType = ATOM_TYPE_UNKNOWN;

    // If atom is an extension, check the extension file name to see if it's a pushed atom.
    if (atomField->is_extension()) {
        if (atomField->number() >= PULLED_ATOMS_START && atomField->number() <= PULLED_ATOMS_END) {
            atomType = ATOM_TYPE_PULLED;
        } else {
        ...
        if (oneofAtom != nullptr) {
            if (oneofAtom->name() == ONEOF_PUSHED_ATOM_NAME) {
                atomType = ATOM_TYPE_PUSHED;
            } else if (oneofAtom->name() == ONEOF_PULLED_ATOM_NAME) {
                atomType = ATOM_TYPE_PULLED;
            }
        }
    }
