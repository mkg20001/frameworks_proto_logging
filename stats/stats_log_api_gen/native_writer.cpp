...
static int write_native_build_vendor_atom_methods(FILE* out,
    ...
    for (auto signatureInfoMapIt = signatureInfoMap.begin();
        ...
        fprintf(out, "    vector<VendorAtomValue> values;\n");
        fprintf(out, "    VendorAtomValue tmp;\n");
        ...
        for (vector<java_type_t>::const_iterator arg = signature.begin(); arg != signature.end();
            ...
            switch (*arg) {
                ...
                case JAVA_TYPE_BYTE_ARRAY:
                    fprintf(out, "    {\n");
                    fprintf(out, "    vector<uint8_t> arrayValue(\n");
                    fprintf(out, "        reinterpret_cast<const uint8_t*>(arg%d.arg),\n", argIndex);
                    fprintf(out, "        reinterpret_cast<const uint8_t*>(arg%d.arg) + arg%d.arg_length);\n", argIndex, argIndex);
                    fprintf(out, "    tmp.set<VendorAtomValue::byteArrayValue>(std::move(arrayValue));\n");
                    fprintf(out, "    values.push_back(tmp);\n");
                    fprintf(out, "    }\n");
                    break;
                case JAVA_TYPE_BOOLEAN:
                    fprintf(out, "    tmp.set<VendorAtomValue::boolValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_INT:
                    [[fallthrough]];
                case JAVA_TYPE_ENUM:
                    fprintf(out, "    tmp.set<VendorAtomValue::intValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_FLOAT:
                    fprintf(out, "    tmp.set<VendorAtomValue::floatValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_LONG:
                    fprintf(out, "    tmp.set<VendorAtomValue::longValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_STRING:
                    if (argIndex > 1) {
                        fprintf(out, "    tmp.set<VendorAtomValue::stringValue>(arg%d);\n",
                                argIndex);
                        fprintf(out, "    values.push_back(tmp);\n");
                    } else {
                        fprintf(stderr, "Skip reverse_domain_name field\n");
                    }
                    break;
                case JAVA_TYPE_BOOLEAN_ARRAY:
                    fprintf(out, "    {\n");
                    ...
                    fprintf(out, "    tmp.set<VendorAtomValue::repeatedBoolValue>(std::move(arrayValue));\n");
                    fprintf(out, "    values.push_back(tmp);\n");
                    fprintf(out, "    }\n");
                    break;
                case JAVA_TYPE_INT_ARRAY:
                    [[fallthrough]];
                case JAVA_TYPE_ENUM_ARRAY:
                    fprintf(out, "    tmp.set<VendorAtomValue::repeatedIntValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_FLOAT_ARRAY:
                    fprintf(out, "    tmp.set<VendorAtomValue::repeatedFloatValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_LONG_ARRAY:
                    fprintf(out, "    tmp.set<VendorAtomValue::repeatedLongValue>(arg%d);\n", argIndex);
                    fprintf(out, "    values.push_back(tmp);\n");
                    break;
                case JAVA_TYPE_STRING_ARRAY:
                    fprintf(out, "    {\n");
                    ...
                    fprintf(out, "    tmp.set<VendorAtomValue::repeatedStringValue>(std::move(arrayValue));\n");
                    fprintf(out, "    values.push_back(tmp);\n");
                    fprintf(out, "    }\n");
                    break;
                default:
