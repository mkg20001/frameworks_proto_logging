...
// static void write_rust_header_import(FILE* out, string headerCrate){
//     fprintf(out, "use %s::{StatsError, StatsResult, Atoms};\n", headerCrate.c_str());
// }
...
static int write_rust_method_body(FILE* out, const AtomDecl& atomDecl,
    ...
    fprintf(out, "            AStatsEvent_setAtomId(__event, Atoms::%s as u32);\n",
...
static void write_rust_struct(FILE* out, const shared_ptr<AtomDecl>& atomDecl,
    ...
    if (isPush) {
        ...
        if (lifetime) {
            fprintf(out, "    impl<'a> Stat for %s<'a> {\n",
                    make_camel_case_name(atomDecl->name).c_str());
        } else {
            fprintf(out, "    impl Stat for %s {\n",
                    make_camel_case_name(atomDecl->name).c_str());
        }
    }
