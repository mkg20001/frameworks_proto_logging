...
static void write_rust_method_signature(FILE* out, const char* namePrefix,
    ...
    fprintf(out, "    ) -> %s::StatsResult", headerCrate.c_str());
    if (isCode) {
        fprintf(out, " {");
    }
    fprintf(out, "\n");
...
static void write_rust_atom_constants(FILE* out, const Atoms& atoms,
    ...
    for (const shared_ptr<AtomDecl>& atomDecl : atoms.decls) {
        ...
        bool isSupported = write_rust_usage(out, "// stats_write", atomDecl, attributionDecl, false, headerCrate);
        if (!isSupported) {
            continue;
        }
...
static int write_rust_method_body(FILE* out, const AtomDecl& atomDecl,
    ...
    if (atomDecl.oneOfName == ONEOF_PUSHED_ATOM_NAME) {
        ...
        fprintf(out, "            if __ret >= 0 { %s::StatsResult::Ok(()) }"
                " else { Err(StatsError::Return(__ret)) }\n", headerCrate.c_str());
    } else {
...
static void write_rust_struct(FILE* out, const shared_ptr<AtomDecl>& atomDecl,
                              const AtomDecl& attributionDecl, string headerCrate) {
    // Write the struct.
...
static int write_rust_stats_write_atoms(FILE* out, const AtomDeclSet& atomDeclSet,
    ...
    for (const auto &atomDecl : atomDeclSet) {
        ...
        int ret = write_rust_stats_write_method(out, atomDecl, attributionDecl, minApiLevel, headerCrate);
        if (ret != 0) {
            return ret;
        }
        auto nonChained = nonChainedAtomDeclSet.find(atomDecl);
        if (nonChained != nonChainedAtomDeclSet.end()) {
            write_rust_stats_write_non_chained_method(out, *nonChained, attributionDecl, headerCrate);
        }
        fprintf(out, "}\n");
    }
    return 0;
...
void write_stats_log_rust_header(FILE* out, const Atoms& atoms, const AtomDecl& attributionDecl, string headerCrate) {
    // Print prelude
...
int write_stats_log_rust(FILE* out, const Atoms& atoms, const AtomDecl& attributionDecl,
                         const int minApiLevel, string headerCrate) {
    //Print prelude
    fprintf(out, "// This file is autogenerated.\n");
    fprintf(out, "\n");
